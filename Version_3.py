...
def main(file_path, n_components=4):
    ...
    # Manually select the fetal ECG component
    fetal_ecg_component_index = int(input("Enter the index of the fetal ECG component (0-based): "))

    # Extract fetal ECG signal
    fetal_ecg = ica_maternal[:, fetal_ecg_component_index]

    # Plot the extracted fetal ECG
    plt.figure(figsize=(12, 4))
    plt.plot(fetal_ecg, label="Extracted Fetal ECG", color='r')
    plt.title("Reconstructed Fetal ECG Signal")
    plt.xlabel("Time (samples)")
